...
```{r filtering_and_modules, cache=FALSE,echo=FALSE,results='hide'}
filtered<-sca_alex
...
```{r zlm,dependson="filtering_and_modules", cache=FALSE,echo=7}
...
#### MAST CDR vs SVA and RUV
```{r}
### Surrogate Variable Analysis
library(sva)
library(RUVSeq)
E = (2^exprs(filtered_nobaseline))-1
ng = rowMeans(E>0)
svafit = svaseq(t(round(exprs(filtered_nobaseline)*3e6)),mod = model.matrix(~Stim/Time,cData(filtered_nobaseline)),n.sv = 2)
boxplot(sv~Time/Stim,data.frame(sv=svafit$sv[,2],Stim = cData(filtered_nobaseline)$Stim,Time=cData(filtered_nobaseline)$Time))


### RUVSeq
mat = dcast((melt(lapply(apply(model.matrix(~foo-1,data.frame(foo=interaction(cData(filtered_nobaseline)$Stim,cData(filtered_nobaseline)$Time))),2,function(x)which(x==1)),as.list))),L1~L2)
mat[is.na(mat)]=-1
ruvres = RUVs(x = t(exprs(filtered_nobaseline)), cIdx = 1:8029, k = 2, scIdx = as.matrix(mat[,-1L]),isLog = TRUE)
pdf(file="../inst/extdata/RUVSVAmDC.pdf",width=12,height=3)
par(mfrow=c(1,4))
plot(svafit$sv[,1],cData(filtered_nobaseline)$cngeneson,xlab="SV1",ylab="CDR",col=cData(filtered_nobaseline)$Stim:cData(filtered_nobaseline)$Time,pch=20)
text(y=-0.15,x=-0.1,labels = substitute("rho" == r,list(r=signif(cor(svafit$sv[,1],cData(filtered_nobaseline)$cngeneson),3))))

plot(svafit$sv[,2],cData(filtered_nobaseline)$cngeneson,xlab="SV2",ylab="CDR",col=cData(filtered_nobaseline)$Stim:cData(filtered_nobaseline)$Time,pch=20)
text(y=-0.15,x=-0.1,labels = substitute("rho" == r,list(r=signif(cor(svafit$sv[,2],cData(filtered_nobaseline)$cngeneson),3))))

plot(ruvres$W[,1],cData(filtered_nobaseline)$cngeneson,col=cData(filtered_nobaseline)$Stim:cData(filtered_nobaseline)$Time,xlab="RUV1",ylab="CDR",pch=20)
text(y=-0.15,x=0.25,labels = substitute("rho" == r,list(r=signif(cor(ruvres$W[,1],cData(filtered_nobaseline)$cngeneson),3))))

plot(ruvres$W[,2],cData(filtered_nobaseline)$cngeneson,col=cData(filtered_nobaseline)$Stim:cData(filtered_nobaseline)$Time,xlab="RUV2",ylab="CDR",pch=20)
text(y=-0.15,x=-0.05,labels = substitute("rho" == r,list(r=signif(cor(ruvres$W[,2],cData(filtered_nobaseline)$cngeneson),3))))
dev.off()
###
```
...
```{r any_time_effect,dependson="zlm", cache=FALSE,fig.width=15}
...
```{r gsea,dependson="zlm;filtering_and_modules",cache=FALSE,fig.width=10,fig.height=10,echo=c(3,20)}
#GSEA
options(mc.cores = 7)
...
toplot = toplot[,adjp:= p.adjust(P,"fdr"),.(factor(coefficient %like% "LPS"):factor(coefficient %like%
...
toplot = toplot[,include_sub := adjp<0.01&effectfilter,.(coefficient)]
toplot = toplot[,include := any(include_sub),.(Stim,TERM)]
#BTM
...
MELTED = MELTED[,"component" := (str_split_fixed(variable,"_",2))[,1]]
MELTED = MELTED[,component := ifelse(component == "Z","combined",component)]
levs <-
...
MELTED_PIC = MELTED_PIC[,"component" := (str_split_fixed(variable,"_",2))[,1]]
MELTED_PIC = MELTED_PIC[,component := ifelse(component == "Z","combined",component)]
levs_pic <-
...
MELTED_PAM = MELTED_PAM[,"component" := (str_split_fixed(variable,"_",2))[,1]]
MELTED_PAM = MELTED_PAM[,component := ifelse(component == "Z","combined",component)]
levs_pam <-
...
```{r, dependson="zlm;filtering_and_modules;gsea",cache=FALSE,warning=FALSE,message=FALSE,results='hide'}
options(mc.cores = 7)
...
```{r residual_correlation,dependson="zlm;filtering_and_modules;any_time_effect",cache=FALSE,results='hide'}
...
```{r}
                                             PCbiplot <-
                                                ...
                                                abs(logFC) > FCTHRESHOLD,][order(-abs(logFC),adj)][1:1000,primerid])[1:20]
                                                stim_sel <- "LPS"
                                                shalek_LPS_residual_pca_oneplot <-
                                                PCbiplot(
                                                ...
                                                plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line =
                                                element_line(colour = "black")
                                                ) + aes(col=Time)+geom_point(show_guide=TRUE)
shalek_LPS_residual_pca_oneplot
```

### PC plot of residuals in LPS

```{r pca_residuals,cache=FALSE,dependson="any_time_effect;zlm;filtering_and_modules",results='hide'}
                                              PCbiplot <-
                                                function(PC, x = "PC1", y = "PC2", colors = c('black', 'black', 'red', 'red'),arrow_colors =
                                                "red",point_colors = "black",arrow_alpha = 0.75,meta = NULL,gene_subset =
                                                NULL,scale = 0.7) {
...
```{r viz_pam,cache=FALSE,dependson="filtering_and_modules;zlm_pam",results='hide'}
...
```{r viz_pic,cache=FALSE,dependson="filtering_and_modules;zlm_pic",results='hide'}
