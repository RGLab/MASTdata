---
title: 'PRIST: A flexible statistical framework for assessing transcriptional changes
...
install_github('RGLab/MAST', ref = "master")
#install_version("arm","1.7-07")
```
...
echo = FALSE,eval = TRUE,message = FALSE,error = FALSE,warning = FALSE
))
plotheme <-
theme(
...
eid <-
select(
TxDb.Hsapiens.UCSC.hg19.knownGene,keys = fData(sca_mait)$entrez,keytype =
"GENEID",columns = c("GENEID","TXNAME")
)
...
```{r, dependson="data", cache=FALSE,results='hide'}
# thresholding
...
```{r zlm, dependson="data",results='hide'}
options(mc.cores=7)
...
p3 = ggplot(toplot.combined[pdev > 0]) + stat_ecdf() + aes(col = Component,x = pdev) +
  facet_wrap( ~Dataset) + theme_linedraw() + plotheme + coord_trans(ytrans =
  MIMOSA:::asinh_trans(1)) + scale_y_continuous("Fraction of Genes") +
  theme(axis.text.x = element_text(angle = 45,hjust = 1)) + scale_x_continuous("Percent Deviance Explained by CDR")

pdf(file = "../inst/extdata/output/Supplementary_Figure_4.pdf",width = 4,height =
      3)
      print(p3)
      dev.off()
```
...
sets <-
sapply(t_stat_comb[order(-abs(t_stat_comb$Z),t_stat_comb$adj),set][1:14],function(x)
Kmisc::wrap(x,25))
sets <- gsub("\n$","",gsub(".\\(.+$","",sets[!sets %like% "TBA"]))[-2]
sets=sets[-c(8)]
gsea_scores <-
ggplot(scores[,set := gsub("\n$","",gsub(".\\(.+$","",set))][set %in% sets,list(score =
...
```{r figure3b}
combined_gsea <- calcZ(gsea,testType = "normal",combined = "stouffer")
twocomponent_gsea <- calcZ(gsea,testType = "t")
gsea_result <-
data.table(merge(dcast(melt(combined_gsea),set ~ metric),dcast(
melt(twocomponent_gsea),set ~ comp + metric
),by = "set"))
gsea_result = gsea_result[,adjP := p.adjust(P,"fdr")]
ylevels <-
as.character(gsea_result[order(Z,adjP)][adjP < 0.01 &
!set %like% "TBA"&!set %like% "B cell",set])

gsea_heatmap <-
ggplot(data.table:::melt.data.table(gsea_result[order(Z,adjP)][adjP < 0.01 &
!set %like% "TBA"&!set %like% "B cell" ],id = "set")[variable %like% "Z"]) + aes(y = set,x = variable,fill =
value) + geom_tile() + scale_y_discrete(limits = ylevels,labels = gsub(" \\(.+$","",ylevels)) +
scale_x_discrete(
"Model Component",limits = c("cont_Z","disc_Z","Z"),labels = c("Continuous","Discrete","Combined")
...
pdf("../inst/extdata/output/Figure4.pdf",width=15,height=7.5)
...
png("../inst/extdata/output/Figure4.png",width=15*300,height=7.5*300,res=300)
layout<-cbind(cbind(matrix(2,ncol=1,nrow=10),matrix(1,nrow=10,ncol=9)),cbind(matrix(3,nrow=10,ncol=9),matrix(2,ncol=1,nrow=10)))
lay<-grid.layout(nrow = nrow(layout),ncol=ncol(layout))
grid.newpage()
vp<-viewport(layout=lay)
...
```{r,fig.cap='Figure 4',fig.width=12,fig.height=8}
layout<-cbind(cbind(matrix(2,ncol=1,nrow=10),matrix(1,nrow=10,ncol=9)),cbind(matrix(3,nrow=10,ncol=9),matrix(2,ncol=1,nrow=10)))
lay<-grid.layout(nrow = nrow(layout),ncol=ncol(layout))
grid.newpage()
vp<-viewport(layout=lay)
...
```{r pca,dependson="data;residuals;",fig.width=10,include=FALSE}
library(grid)
...
pdf(file="../inst/extdata/output/Figure3.pdf",width=15,height=5)
grid.newpage()
lay<-grid.layout(nrow(lout), ncol(lout))
pushViewport(viewport(layout=lay,name = "top"))
...
lout<-matrix(rep(c(1,2,3),each=100),ncol=30)
lout<-rbind(rep(4,30),lout,rep(4,30))
png(file="../inst/extdata/output/Figure3.png",width=300*15,height=300*5,res=300)
grid.newpage()
lay<-grid.layout(nrow(lout), ncol(lout))
pushViewport(viewport(layout=lay,name = "top"))
matchind<-which(lout==c(1),T)
...
# cache(p<-ggplot(C3)+aes(x=rho,group=model,col=model)+geom_density()+theme_linedraw()+ggtitle("Unstimulated")+theme(aspect.ratio=1))
# cache(p.s<-ggplot(C3.s)+aes(x=rho,group=model,col=model)+geom_density()+theme_linedraw()+ggtitle("Stimulated")+theme(aspect.ratio=1))
...
pdf("../inst/extdata/output/Supplementary_Figure_9.pdf",width=24,height=7)
grid.newpage()
...
t_stat_unstim_comb[p.adjust(disc_P,"fdr")<0.1, ][order(disc_Z)]
...
lim<-t_stat_stim_comb[!is.nan(P)][p.adjust(P,"fdr")<0.1 ][order(Z),set]
ggplot(setDT(melt(t_stat_stim_comb[!is.nan(P)][p.adjust(P,"fdr")<0.1 ][order(Z)]))[variable%like%"Z"])+geom_tile(aes(x=variable,y=set,fill=value))+scale_y_discrete(limits=lim)+scale_fill_gradient2("GSEA Z-score",low = ("#E9A3C9"),mid=("#F7F7F7"),high=("#A1D76A"),space="Lab")+scale_x_discrete("Component",labels=c("Continous","Discrete","Combined"))+theme_linedraw()+plotheme+theme(axis.text.x=element_text(angle=90,hjust=1))+ggtitle("GSEA of non-responding MAITs vs stimulated MAITs")

lim2<-t_stat_unstim_comb[p.adjust(disc_P,"fdr")<0.1 ][order(disc_Z),set]
ggplot(setDT(melt(t_stat_unstim_comb[p.adjust(disc_P,"fdr")<0.1 ][order(Z)]))[variable%like%"Z"])+geom_tile(aes(x=variable,y=set,fill=value))+scale_y_discrete(limits=lim2)+scale_fill_gradient2("GSEA Z-score",low = ("#E9A3C9"),mid=("#F7F7F7"),high=("#A1D76A"),space="Lab")+scale_x_discrete("Component",labels=c("Continous","Discrete","Combined"))+theme_linedraw()+plotheme+theme(axis.text.x=element_text(angle=90,hjust=1))+ggtitle("GSEA of non-responding MAITs vs unstimulated MAITs")
ggsave("../inst/extdata/output/Supplementary_Figure_10.pdf")
```
...
layer(sca_mait)<-"tpm"
melted<-data.table(melt(sca_mait))
...
pcs<-prcomp(exprs(sca))$x
scree_mait<-lapply(list(prcomp(exprs(sca))$sdev^2),function(x)x/sum(x))[[1]][1:8]
mait_pc<-data.frame(pcs[,1:2],ngeneson=data.table(melt(sca))[,list(ngeneson=mean(tpm>0)),wellKey][,ngeneson],experiment="MAIT",stim=cData(sca)[,"condition"])
...
f2<-ggplot(data.table(mait_pc))+geom_point(aes(y=PC2,x=ngeneson,col=stim),show_guide=FALSE)+facet_wrap(~experiment,scales="free")+theme_linedraw()+scale_color_manual(values=colors)+scale_y_continuous(paste0("PC2 (",sprintf("%2.1f",100*scree_mait[2]),"%)"))+scale_x_continuous("Fraction of genes expressed")
f1<-ggplot(data.table(mait_pc))+geom_point(aes(y=PC1,x=ngeneson,col=stim),show_guide=FALSE)+facet_wrap(~experiment,scales="free")+theme_linedraw()+scale_color_manual(values=colors)+scale_y_continuous(paste0("PC1 (",sprintf("%2.1f",100*scree_mait[1]),"%)"))+scale_x_continuous("Fraction of genes expressed")
f3<-ggplot(data.table(shalek_pc))+geom_point(aes(y=PC2,x=ngeneson,col=stim),show_guide=FALSE)+facet_wrap(~experiment,scales="free")+theme_linedraw()+scale_color_manual(values=colors2)+scale_y_continuous(paste0("PC2 (",sprintf("%2.1f",100*scree_shalek[2]),"%)"))+scale_x_continuous("Fraction of genes expressed")
f4<-ggplot(data.table(shalek_pc))+geom_point(aes(y=PC1,x=ngeneson,col=stim),show_guide=FALSE)+facet_wrap(~experiment,scales="free")+theme_linedraw()+scale_color_manual(values=colors2)+scale_y_continuous(paste0("PC1 (",sprintf("%2.1f",100*scree_shalek[1]),"%)"))+scale_x_continuous("Fraction of genes expressed")
