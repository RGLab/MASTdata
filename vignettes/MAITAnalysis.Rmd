...
sca <- sca_mait
```
Next, let's load the data.  It consists of a matrix of log2 + 1 transcripts per million, as output by RSEM.
Internally, we use a couple of packages to facilitate this workflow, though we will probably transition to pseudo-alignment based methods shortly.
...
```{r zlm, dependson="data",results='hide',echo=6}
